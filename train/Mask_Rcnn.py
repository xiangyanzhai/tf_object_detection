...
from object_detection.tool.tf_ATC_FPN import AnchorTargetCreator
from object_detection.tool.tf_PC_FPN import ProposalCreator
from object_detection.tool.tf_PTC_mask_test import ProposalTargetCreator
from object_detection.tool.read_Data_mask import readData
...
class Mask_rcnn_resnet_101():
    ...
    def train(self):
        ...
        shard_nums = self.config.gpus * self.config.batch_size
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(00000, 90010 * x):
                ...
                if (i + 1) % 5000 == 0 or ((i + 1) % 1000 == 0 and i < 10000):
                    saver.save(sess, os.path.join('./models/', 'Mask_Rcnn.ckpt'), global_step=i + 1)
